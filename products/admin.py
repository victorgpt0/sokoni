...
class ProductImageInline(admin.TabularInline):
    ...
    def image_preview(self, obj):
        if obj.image:
            return format_html('<img src="{}" style="width: auto; height: 50px;"/>', obj.image.url)
        return 'No image'
...
class ProductAdmin(admin.ModelAdmin):
    list_display = ('name', 'category', 'price', 'stock_quantity','is_active', 'is_featured', 'created_at', 'updated_at')
    list_filter = ('category', 'is_active', 'is_featured', 'created_at', 'updated_at')
    prepopulated_fields = {'slug': ('name',)}
    search_fields = ('name', 'description', 'category__name', 'sku')
    ordering = ('-created_at',)
    inlines = [ProductImageInline]
    readonly_fields = ('created_at', 'updated_at', 'discount_percentage_display', 'is_on_sale_display', 'is_in_stock_display', 'created_by', 'updated_by',)
    ...
    fieldsets = (
        ...
        ('Pricing', {
            'fields': ('price', 'compare_price', 'discount_percentage_display', 'is_on_sale_display')
        }),
        ('Inventory', {
            'fields': ('sku', 'stock_quantity', 'is_in_stock_display', 'weight', 'dimensions')
        }),
        ('Status', {
...
class ProductImageAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ('created_at', 'updated_at', 'image_preview', 'created_by', 'updated_by',)
