...
class Command(BaseCommand):
    ...
    def create_users(self, count):
        ...
        first_names = [
            'John', 'Jane', 'Michael', 'Sarah', 'David', 'Emily', 'Chris', 'Ashley',
            'Matthew', 'Jessica', 'Andrew', 'Amanda', 'Joshua', 'Stephanie', 'Daniel',
            'Nicole', 'Anthony', 'Elizabeth', 'Mark', 'Helen', 'Kevin', 'Michelle',
            'Brian', 'Kimberly', 'Steven', 'Donna', 'Joseph', 'Carol', 'Thomas', 'Ruth'
        ]
    ...
    def create_addresses(self, users):
        ...
        street_names = [
            'Kimathi Street', 'Uhuru Highway', 'Kenyatta Avenue', 'Moi Avenue',
            'Harambee Avenue', 'University Way', 'Tom Mboya Street', 'Jogoo Road',
            'Ngong Road', 'Waiyaki Way', 'Outer Ring Road', 'Thika Road'
        ]
        ...
        for user in users:
            ...
            for i in range(num_addresses):
                address = CustomerAddress.objects.create(
                    ...
                    address_line_2=f"Apt {random.randint(1, 50)}" if random.choice([True, False]) else "",
                    city=random.choice(kenyan_cities),
                    state=random.choice([
                        'Nairobi', 'Mombasa', 'Kisumu', 'Nakuru', 'Uasin Gishu',
                        'Kiambu', 'Kilifi', 'Trans Nzoia', 'Garissa', 'Kakamega'
                    ]),
    ...
    def create_products(self, categories, count):
        ...
        product_templates = {
            'Electronics': [
                {'name': 'iPhone 14 Pro', 'price': 999.99, 'description': 'Latest iPhone with Pro camera system'},
                {'name': 'Samsung Galaxy S23', 'price': 899.99, 'description': 'Flagship Android smartphone'},
                {'name': 'MacBook Air M2', 'price': 1199.99, 'description': 'Apple laptop with M2 chip'},
                {'name': 'Dell XPS 13', 'price': 999.99, 'description': 'Premium Windows laptop'},
                {'name': 'iPad Pro 12.9"', 'price': 1099.99, 'description': 'Professional tablet'},
                {'name': 'Sony WH-1000XM4', 'price': 349.99, 'description': 'Noise-canceling headphones'},
                {'name': 'Canon EOS R5', 'price': 3899.99, 'description': 'Professional mirrorless camera'},
                {'name': 'Nintendo Switch OLED', 'price': 349.99, 'description': 'Gaming console'},
            ],
            'Fashion': [
                {'name': 'Levi\'s 501 Jeans', 'price': 89.99, 'description': 'Classic straight-leg jeans'},
                {'name': 'Nike Air Max 90', 'price': 119.99, 'description': 'Iconic sneakers'},
                {'name': 'Ray-Ban Aviator', 'price': 159.99, 'description': 'Classic aviator sunglasses'},
                {'name': 'Polo Ralph Lauren Shirt', 'price': 79.99, 'description': 'Classic polo shirt'},
                {'name': 'Coach Leather Handbag', 'price': 299.99, 'description': 'Premium leather handbag'},
                {'name': 'Rolex Submariner', 'price': 8995.99, 'description': 'Luxury dive watch'},
            ],
            'Home & Garden': [
                {'name': 'IKEA BILLY Bookshelf', 'price': 59.99, 'description': 'Adjustable bookshelf'},
                {'name': 'KitchenAid Stand Mixer', 'price': 379.99, 'description': 'Professional stand mixer'},
                {'name': 'Dyson V15 Vacuum', 'price': 749.99, 'description': 'Cordless vacuum cleaner'},
                {'name': 'Weber Genesis Grill', 'price': 899.99, 'description': 'Gas barbecue grill'},
            ],
            'Sports & Fitness': [
                {'name': 'Peloton Bike+', 'price': 2495.99, 'description': 'Smart exercise bike'},
                {'name': 'Nike Dri-FIT T-Shirt', 'price': 29.99, 'description': 'Moisture-wicking athletic shirt'},
                {'name': 'Wilson Tennis Racket', 'price': 129.99, 'description': 'Professional tennis racket'},
                {'name': 'Yeti Cooler 45', 'price': 299.99, 'description': 'Insulated cooler'},
            ],
            'Books & Media': [
                {'name': 'The Great Gatsby', 'price': 12.99, 'description': 'Classic American novel'},
                {'name': 'Dune (4K Blu-ray)', 'price': 24.99, 'description': 'Sci-fi movie in 4K'},
                {'name': 'Taylor Swift - Midnights', 'price': 14.99, 'description': 'Latest album on vinyl'},
            ],
            'Health & Beauty': [
                {'name': 'CeraVe Moisturizer', 'price': 16.99, 'description': 'Daily facial moisturizer'},
                {'name': 'Olaplex Hair Treatment', 'price': 28.99, 'description': 'Professional hair repair'},
                {'name': 'Vitamin D3 Supplements', 'price': 19.99, 'description': 'Daily vitamin supplements'},
            ]
        }
        ...
        for i in range(count):
            ...
            template = None
            for parent_name, category_dict in product_templates.items():
            ...
            product = Product.objects.create(
                name=name,
                slug=slugify(name),
                description=template['description'] + f" - Perfect for {category_name.lower()}.",
                category=category,
                price=Decimal(str(template['price'])),
                compare_price=Decimal(str(template['price'] * random.uniform(1.1, 1.5))) if random.choice([True, False]) else None,
    ...
    def create_reviews(self, products, users):
        ...
        review_titles = [
            ...
            "Perfect for my needs", "Amazing value", "Outstanding service",
            "Could be better", "Not what I expected", "Decent product",
            "Fantastic purchase", "Really impressed", "Good but expensive",
        ...
        reviews = []
        for product in random.sample(products, min(len(products), 40)):  # Review 40 random products
