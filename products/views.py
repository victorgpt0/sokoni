...
class ProductDetailView(DetailView):
    ...
    def get_queryset(self):
        ...
        if self.request.user.is_authenticated:
            ...
            ).prefetch_related("images", "reviews__customer__user", "category")
        else:
        ...
        products_data.append(
            {
                ...
                "compare_price": (
                    str(product.compare_price) if product.compare_price else None
                ),
...
def search_suggestions_view(request):
    ...
    # Product suggestions
    products = Product.objects.filter(name__icontains=query, is_active=True).values(
        "name", "slug"
    )[:5]
    ...
    suggestions = {"products": list(products), "categories": list(categories)}
...
def product_manage_images(request, slug):
    ...
    if request.method == "POST":
        formset = ProductImageFormSet(
            request.POST, request.FILES, queryset=product.images.all()
        )
        if formset.is_valid():
...
def my_products(request):
    ...
    context = {"products": products_page, "title": "My Products"}
