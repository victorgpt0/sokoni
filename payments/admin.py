...
class PaymentAdmin(admin.ModelAdmin):
    list_display = ('order_number', 'customer_info', 'amount_display', 'payment_method', 'status_display', 'created_at', 'paid_at')
    list_filter = ('status', 'payment_method', 'created_at', 'paid_at', 'failed_at')
    search_fields = ('order__order_number', 'order__customer__user__email', 'order__customer__user__username', 'paystack_reference')
    readonly_fields = ('order', 'amount', 'currency', 'created_at', 'updated_at', 'created_by', 'updated_by')
    ...
    fieldsets = (
        ...
        ('Paystack Information', {
            'fields': ('paystack_reference', 'paystack_access_code', 'paystack_transaction_id'),
            'classes': ('collapse',)
        }),
        ('Payment Status', {
            'fields': ('paid_at', 'failed_at', 'failure_reason')
        }),
        ('Gateway Response', {
    ...
    def amount_display(self, obj):
        color = 'green' if obj.is_successful else 'red' if obj.status == 'failed' else 'orange'
        return format_html(
    ...
    actions = ['mark_as_completed', 'mark_as_failed', 'mark_as_refunded', 'resend_confirmation_email']
    ...
    def resend_confirmation_email(self, request, queryset):
        ...
        for payment in queryset.filter(status='completed'):
            try:
                ...
                self.message_user(request, f'Failed to send email for payment {payment.id}: {e}', level='ERROR')
        self.message_user(request, f'Confirmation emails sent for {count} payments.')
    resend_confirmation_email.short_description = "Resend confirmation emails for selected payments"
...
class PaymentAttemptAdmin(admin.ModelAdmin):
    list_display = ('payment_order', 'paystack_reference', 'status_display', 'created_at')
    list_filter = ('status', 'created_at')
    search_fields = ('payment__order__order_number', 'paystack_reference')
    readonly_fields = ('payment', 'paystack_reference', 'status', 'gateway_response', 'created_at', 'updated_at', 'created_by', 'updated_by')
    date_hierarchy = 'created_at'
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related(
            'payment__order__customer__user'
        )
