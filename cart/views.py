    ...
    if 'applied_coupon_code' in request.session:
        try:
            applied_coupon = Coupon.objects.get(
                code=request.session['applied_coupon_code'],
                is_active=True
            )
...
def apply_coupon_view(request):
    ...
    try:
        ...
        if coupon.discount_type == 'percentage':
            ...
            messages.success(request, f'Coupon applied! {coupon.discount_value}% discount (${discount:.2f})')
        else:
            messages.success(request, f'Coupon applied! ${coupon.discount_value} discount')
...
def cart_mini_view(request):
    ...
    cart = get_or_create_cart(request)
    cart_items = cart.cart_items.select_related('product').prefetch_related('product__images')[:5]
...
def merge_carts(user):
    ...
    session_key = None
    if hasattr(user, '_session_key'):
        session_key = user._session_key
