...
def validate_coupon_api(request):
    ...
    try:
        ...
        if not coupon.is_valid:
            return JsonResponse(
                {"valid": False, "error": "This coupon has expired or is not active"}
            )
...
def coupon_list_view(request):
    ...
    if search_query:
        coupons = coupons.filter(
            Q(code__icontains=search_query) | Q(discount_type__icontains=search_query)
        )
    ...
    if status == "active":
        ...
        coupons = [
            c for c in coupons if c.usage_limit and c.used_count >= c.usage_limit
        ]
...
def coupon_stats_view(request, coupon_id):
    ...
    stats = {
        ...
        "usage_percentage": (
            (coupon.used_count / coupon.usage_limit * 100) if coupon.usage_limit else 0
        ),
        "is_active": coupon.is_active,
        "is_expired": timezone.now() > coupon.valid_to if coupon.valid_to else False,
        "days_remaining": (
            (coupon.valid_to - timezone.now()).days
    ...
    try:
        ...
        if order_total < coupon.minimum_amount:
            return (
                False,
                0,
                f"Minimum order amount of KES {coupon.minimum_amount} required",
            )
