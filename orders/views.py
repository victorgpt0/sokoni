...
def checkout_view(request):
    ...
    shipping_addresses = CustomerAddress.objects.filter(
        customer=request.user.customer, type="shipping"
    ).order_by("-is_default", "-created_at")

    billing_addresses = CustomerAddress.objects.filter(
        customer=request.user.customer, type="billing"
    ).order_by("-is_default", "-created_at")
...
def create_order_view(request):
    ...
    try:
        shipping_address = CustomerAddress.objects.get(
            id=shipping_address_id, customer=request.user.customer, type="shipping"
        )
        billing_address = CustomerAddress.objects.get(
            id=billing_address_id, customer=request.user.customer, type="billing"
        )
        ...
        return JsonResponse(
            {
                ...
                "payment_url": reverse(
                    "payments:process_payment", args=[order.order_number]
                ),
            }
        )
...
def order_tracking_view(request, order_number):
    ...
    try:
        order = Order.objects.get(
            order_number=order_number, customer__user__email=email
        )
        ...
        messages.error(
            request, "Order not found. Please check your order number and email."
        )
        return render(request, "orders/track_order_form.html")
...
def order_invoice_view(request, order_number):
    ...
    if request.user.is_authenticated:
        ...
        order = get_object_or_404(
            Order, order_number=order_number, customer__user__email=email
        )
